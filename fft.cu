...
// a.conj @ b
template <typename Layout>
__device__ inline void complex_conj_matmul(
    fragment16<matrix_a, row_major> &a_re,
    fragment16<matrix_a, row_major> &a_im,
    fragment16<matrix_b, Layout> &b_re,
    fragment16<matrix_b, Layout> &b_im,
...
#pragma unroll
...

    {
        ...
        // frequency domain kernel multiplication
        fragment16<accumulator> frag_kf_re;
        fragment16<accumulator> frag_kf_im;
        load_matrix_sync(frag_kf_re, gmem_kf_re + b * R * R, R, layout_t::mem_col_major);
        load_matrix_sync(frag_kf_im, gmem_kf_im + b * R * R, R, layout_t::mem_col_major);
        for (int i = 0; i < frag_out_re.num_elements; i++)
        {
            auto v = complex_mul({frag_out_re.x[i], frag_out_im.x[i]}, {frag_kf_re.x[i], frag_kf_im.x[i]});
            frag_out_re.x[i] = v.x;
            frag_out_im.x[i] = v.y;
        }
        ...
        // if we can convert accumulator to matrix_b, store-load pair with same layout can be canceled
        store_matrix_sync(&x_re[b][0][0], frag_out_re, R + PAD2, layout_t::mem_col_major);
        store_matrix_sync(&x_im[b][0][0], frag_out_im, R + PAD2, layout_t::mem_col_major);
    }
    ...
    // symmetric & unitary => inverse = conjugate
    // still use formal transposition to cancel the digit reversal permutation
    {
        // x = einsum('ij,bcj->bci', F.T, x)
        unsigned int b = warp_id;
        // perform x[b, :, :] = einsum('ij,cj->ci', F.T, x[b, :, :])
        ...
        load_matrix_sync(frag_x_im, &x_im[b][0][0], R + PAD2);
        // load in col_major: cj -> jc
        complex_conj_matmul(F_re, F_im, frag_x_re, frag_x_im, frag_out_re, frag_out_im);
...
#pragma unroll
        for (int i = 0; i < frag_out_re.num_elements; i++)
        {
            ...
            // is moved before col_major store: ic -> ci
            // => row-col exchanged
            auto row = static_cast<unsigned int>(idx_col.x[i]);
            auto col = static_cast<unsigned int>(idx_row.x[i]);
            unsigned int idx = (b * R * R) + (row * R) + col; // [b, :, :]
            idx = idx % (R * Ns);
            auto t = static_cast<float>((idx / Ns) * (idx % Ns)) * unit;
            // conjugate
...
#pragma unroll
        for (int i = 0; i < frag_out_re.num_elements; i++)
        {
            auto row = static_cast<unsigned int>(idx_row.x[i]);
            auto col = static_cast<unsigned int>(idx_col.x[i]);
            unsigned int idx = (b * R * R) + (row * R) + col; // [b, :, :] (different!)
            // idx = idx % (R * Ns); // idx < N, no-op
            auto t = static_cast<float>((idx / Ns) * (idx % Ns)) * unit;
            // conjugate
            auto v = complex_mul({frag_out_re.x[i], frag_out_im.x[i]}, {__cosf(t), __sinf(t)});
...

    {
        ...
        load_matrix_sync(frag_x_re, &x_re[0][b][0], (R + PAD1) * (R + PAD2));
        load_matrix_sync(frag_x_im, &x_im[0][b][0], (R + PAD1) * (R + PAD2));
        complex_conj_matmul(F_re, F_im, frag_x_re, frag_x_im, frag_out_re, frag_out_im);

        store_matrix_sync(gmem_x_re + b * R, frag_out_re, R * R, layout_t::mem_row_major);
        store_matrix_sync(gmem_x_im + b * R, frag_out_im, R * R, layout_t::mem_row_major);
    }
}
